...
class TestAdminCategories:
    ...
    async def test_admin_categories_function_success(self, bypass_admin_auth):
        ...
        mock_categories = [MagicMock(to_dict=lambda: {"pkid": 1, "name": "Technology"})]
    ...
    async def test_admin_categories_table_with_counts(
        self, mock_db_ops, client, bypass_admin_auth
    ):
        ...
        mock_db_ops.read_query.side_effect = [
            mock_categories,
            mock_posts,
            mock_weblinks,
        ]
...
class TestAdminUserManagement:
    ...
    @pytest.mark.asyncio
    @patch("src.endpoints.admin.db_ops")
    async def test_admin_user_view_client(
        self, mock_db_ops, client, bypass_admin_auth, mock_user
    ):
        """Test admin user view via client."""
        mock_db_ops.read_one_record = AsyncMock(
            return_value=MagicMock(to_dict=lambda: mock_user)
        )
    ...
    async def test_admin_user_access_control_success(self, bypass_admin_auth):
        ...
        form_data = {"is_admin": ["on"], "is_locked": [""]}
        ...
        with patch("src.endpoints.admin.db_ops") as mock_db:
            ...
            result = await admin_update_user_access(
                mock_request, update_user_id, mock_user_info
            )
...
class TestAdminAIFunctionality:
    ...
    @pytest.mark.asyncio
    @patch("src.endpoints.admin.db_ops")
    async def test_admin_note_ai_check_client(
        self, mock_db_ops, client, bypass_admin_auth
    ):
    ...
    async def test_admin_note_ai_check_user_process(self, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.db_ops") as mock_db:
            ...
            result = await admin_note_ai_check_user(
                user_id, mock_background_tasks, mock_user_info
            )
    ...
    async def test_admin_weblink_fix_user_process(self, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.db_ops") as mock_db:
            ...
            result = await admin_weblink_fix_user(
                user_id, mock_background_tasks, mock_user_info
            )
    ...
    async def test_admin_note_ai_check_database_error(self, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.db_ops") as mock_db:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await admin_note_ai_check_user(
                    user_id, mock_background_tasks, mock_user_info
                )
    ...
    def test_admin_note_ai_check_user_client(self, client, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.db_ops.read_query") as mock_read:
            mock_read.return_value = [
                MagicMock(
                    to_dict=lambda: {"pkid": "note-123", "note": "Test note content"}
                )
            ]
...
class TestAdminErrorHandling:
    ...
    async def test_admin_category_edit_missing_data(self, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.templates") as mock_templates:
            with patch("src.endpoints.admin.db_ops") as mock_db:
                # Mock empty category result when no category_id - create proper mock object
                mock_category_obj = MagicMock()
                mock_category_obj.to_dict.return_value = {"error": "not found"}
                mock_db.read_one_record = AsyncMock(return_value=mock_category_obj)
                mock_templates.TemplateResponse.return_value = MagicMock()
...
class TestAdminIntegration:
    ...
    async def test_admin_ai_workflow_complete(self, bypass_admin_auth):
        ...
        with patch("src.endpoints.admin.db_ops") as mock_db:
            with patch("src.endpoints.admin.templates") as mock_templates:
                ...
                result = await admin_note_ai_check_user(
                    "1", mock_background_tasks, mock_user_info
                )
    ...
    @pytest.mark.asyncio
    @patch("src.endpoints.admin.db_ops")
    async def test_admin_category_create_integration(
        self, mock_db_ops, client, bypass_admin_auth
    ):
