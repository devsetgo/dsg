...
class TestDevtoolsSimple:
    ...
    async def test_post_success(self, mock_logger, mock_check_packages, client):
        ...
        mock_check_packages.return_value = [
            {"package": "requests", "status": "success"}
        ]

        response = client.post("/devtools/pypi/check-list", json=["requests"])
    ...
    async def test_post_with_print_mock(
        self, mock_print, mock_logger, mock_check_packages, client
    ):
    ...
    @patch("src.endpoints.devtools.logger")
    @patch("builtins.print")
    async def test_get_with_print_mock(
        self, mock_print, mock_logger, mock_check_packages, client
    ):
    ...
    @patch("src.endpoints.devtools.check_packages")
    @patch("src.endpoints.devtools.logger")
    async def test_exception_path_coverage(
