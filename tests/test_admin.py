...
class TestAdmin:
    ...
    async def test_admin_dashboard_authorized(
    ...
    async def test_admin_user_view(
        ...
        with patch(
            "src.endpoints.admin.date_functions.timezone_update",
    ...
    async def test_admin_update_user_password(
        ...
        mock_db_ops.read_one_record = AsyncMock(
            return_value=MagicMock(to_dict=lambda: {"pkid": "user-123"})
        )
    ...
    async def test_admin_note_ai_check(self, mock_db_ops, client, bypass_admin_auth):
        ...
        mock_weblinks = [
            MagicMock(to_dict=lambda: {"pkid": "link-1", "user_id": "user-1"})
        ]
