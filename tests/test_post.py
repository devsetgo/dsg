...
#         ]
#         # Make sure the mock returns an awaitable
...
#     @pytest.mark.asyncio
#     async def test_get_posts_standalone(self, client):
...
#     @pytest.mark.asyncio
#     @patch("src.endpoints.blog_posts.db_ops")
#     async def test_edit_post_standalone(
#         self, mock_db_ops, client, bypass_auth, mock_post
#     ):
#         """Test edit post endpoint requires auth."""
#         mock_post_obj = MagicMock()
#         mock_post_obj.to_dict.return_value = mock_post
#         mock_db_ops.read_one_record = AsyncMock(return_value=mock_post_obj)
...
#             AsyncMock(return_value="Jan 1, 2024"),
#         ):
#             response = client.get("/posts/edit/test-id")
#             # Should work with auth bypass and proper mocking
#             assert response.status_code == 200
