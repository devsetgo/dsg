...
class TestPyPICore:
    ...
    async def test_pypi_check_form_post(self, mock_check_packages, client):
        ...
        response = client.post(
            ...
            data={"requirements": "requests==2.28.1\nnumpy>=1.21.0\nflask"},
...
class TestPyPIErrorHandling:
    ...
    @pytest.mark.asyncio
    @patch(
        "src.endpoints.pypi.check_packages",
        side_effect=Exception("Package check failed"),
    )
    ...
    @pytest.mark.asyncio
    @patch(
        "src.endpoints.pypi.db_ops.read_query", side_effect=Exception("Database error")
    )
    ...
    @pytest.mark.asyncio
    @patch(
        "src.endpoints.pypi.get_pypi_metrics", side_effect=Exception("Metrics failed")
    )
...
class TestPyPIIntegration:
    ...
    @patch("src.endpoints.pypi.check_packages")
    @patch("src.endpoints.pypi.db_ops")
    async def test_pypi_full_workflow(
        self, mock_db_ops, mock_check_packages, mock_metrics, client
    ):
