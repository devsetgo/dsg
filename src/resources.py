...
from sqlalchemy import Column, Delete, ForeignKey, Integer, Select, String, Update,JSON,Table,Boolean
...
async def add_interesting_things():
    ...
    # user = await db_ops.get_one_record(Select(User).where(User.user_name == 'Mike'))
    # for item in my_stuff:
    #     category = await db_ops.get_one_record(Select(Categories).where(Categories.name == item['category']))
    #     print(category)
