...
async def read_weblinks_from_file(csv_content: str, user_identifier: str):
    ...
    for i in data:
        ...
        if title =='Not Found':
            title = "Processing"
        link = WebLinks(title=title, summary="Processing",url=i['url'],category=i['category'],public=public, user_id=user_identifier, ai_fix=True)
...
async def ai_process_pkids(pkids: list):
    ...
    for pkid in tqdm(pkids,desc="AI Processing links"):
        ...
        record = await db_ops.read_one_record(Select(WebLinks).where(WebLinks.pkid == pkid))
