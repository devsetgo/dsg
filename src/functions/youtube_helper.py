    ...
    try:
        async with AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to get YouTube metadata for video {video_id}: {response.status_code}")
                return {"title": "YouTube Video", "description": "YouTube video content", "author_name": "", "thumbnail_url": ""}
...
async def get_youtube_summary(url: str, sentence_length: int = 2) -> Dict[str, str]:
    ...
    if not video_id:
        ...
        return {"summary": "YouTube video content - unable to extract video information"}
    ...
    try:
        ...
        fallback_summary = f"YouTube video '{metadata['title']}' by {metadata['author_name']}"
