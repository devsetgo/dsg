...
async def save_uploaded_pdf(uploaded_file, user_id: str) -> dict:
    ...
    try:
        ...
        converted_filepath = OCR_OUTPUT_DIR / f"{job_id}_{timestamp}_converted{file_extension}"
...
async def process_ocr_pdf(job_id: str) -> bool:
    ...
    try:
        ...
        logger.info(f"OCR processing completed for job {job_id} in {processing_duration} seconds")
        return True
...
async def get_ocr_job_status(job_id: str, user_id: str) -> Optional[dict]:
    ...
    try:
        ...
        if job_dict["status"] == "completed":
            job_dict["download_available"] = Path(job_dict["converted_filepath"]).exists()
        else:
...
async def get_user_ocr_jobs(user_id: str, limit: int = 10) -> list:
    ...
    """
    try:
        query = Select(OCRJob).where(OCRJob.user_id == user_id).order_by(
            OCRJob.date_created.desc()
        ).limit(limit)
